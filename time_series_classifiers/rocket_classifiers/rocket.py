...
def read_dataset(path):
    ...
    logger.info("Data path is {}".format(path))
...
class RocketTransformerClassifier:
    ...
    def fit_rocket(self, x_train, y_train, train_pid, kernels=10000):
        ...
        x_training_transform_std = scaler.fit_transform(x_training_transform)
        ...
        classifier.fit(x_training_transform_std, y_train)
    ...
    def predict_rocket(self, x_test, y_test, test_pid, x_val=None, y_val=None):
        ...
        x_test_transform_std = scaler.transform(x_test_transform)
        ...
        # Test Predictions
        predictions = classifier.predict(x_test_transform_std)
        ...
        if x_val.shape[0]:
            ...
            x_val_transform_std = scaler.transform(x_val_transform)
            predictions = classifier.predict(x_val_transform_std)
            ...
            logger.info("Accuracy {}".format(metrics.accuracy_score(y_val, predictions)))
            logger.info("\n Confusion Matrix: \n {}".format(confusion_matrix))
            logger.info("\n Classification report: \n{}".format(classification_report))
...
if __name__ == "__main__":
    ...
    seed_values = rocket_config.as_list("SEED_VALUES")
    ...
    gender_info_df = pd.read_csv(os.path.join(home_path, gender_info_file), sep=",")
...
"""
without normalizing after ROCKET transform
81, 78, 76

normalize after ROCKET transform
85, 87, 87


103007,1899797,191099
"""
